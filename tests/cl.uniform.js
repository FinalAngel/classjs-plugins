...
test('Test for issue #59: radios with same name across different forms', function () {
    ...
    fixture.append([
        '<div class="uniformed">',
            '<form class="form1" action="."><input type="radio" name="x" value="1" checked /><input type="radio" name="x" value="2" /></form>',
            '<form class="form2" action="."><input type="radio" name="x" value="3" checked /><input type="radio" name="x" value="4" /></form>',
            '<div class="not-a-form"><input type="radio" name="x" value="5" checked /><input type="radio" name="x" value="6" /><input type="radio" name="x" value="7" checked /><input type="radio" name="x" value="8" /></div>',
        '</div>'
    ].join(''));

    new Cl.Uniform('.uniformed :radio');

    equal(fixture.find('.form1').find(':radio:checked').val(), '1', 'initial values are ok');
    equal(fixture.find('.form2').find(':radio:checked').val(), '3', 'initial values are ok');
    equal(fixture.find('.not-a-form').find(':radio:checked').val(), '7', 'initial values are ok');

    fixture.find('.form1').find(':radio:last').prop('checked', true).trigger('change');

    equal(fixture.find('.form1').find(':radio:checked').val(), '2', 'change in the form affects only that form');
    equal(fixture.find('.form2').find(':radio:checked').val(), '3', 'change in the form affects only that form');
    equal(fixture.find('.not-a-form').find(':radio:checked').val(), '7', 'change in the form affects only that form');

    fixture.find('.not-a-form').find(':radio:first').prop('checked', true).trigger('change');

    equal(fixture.find('.form1').find(':radio:checked').val(), '2', 'change outside of the form affects only radios that are outside of forms');
    equal(fixture.find('.form2').find(':radio:checked').val(), '3', 'change outside of the form affects only radios that are outside of forms');
    equal(fixture.find('.not-a-form').find(':radio:checked').val(), '5', 'change outside of the form affects only radios that are outside of forms');
});
